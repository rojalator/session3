...
TO-DO for this documentation
============================
The following needs to be done before release (it might also be useful to remove the relevant code
from the auto-generated documentation: however, if someone else decides to help, they'll need the 
code and references):-

 * Formalize the location of the two types of documentation --- where are the docs installed?
 * Remove references (or hide them) for the non-functioning back-ends.
 * Remove references to now discontinued packages (twill, nose, etc.)
 * Check the `Upgrading MySQL`_ section
 * Check the example code in `Using Session3`_
...
`Session3` also provides several (fully functional) [#limited]_ persistent storage back-ends:-
...
than attributes [#dict_note]_. Note that this version (3.0) will only work with Quixote 3.0 as
...
``.setup()``:
	initializes the store.  For MySQL and PostgreSQL, this creates the table.  
	This is meant to be called in your application
...
- ShelveSessionStore is *not* multiprocess safe because it doesn't do file
  ...
  think about using ``fcntl`` in a subclass, see the `fcntl caution`_ below.
