...
def format_page(title:str, content):
    request_html = htmltext('<div style="font-size: smaller;background:#eee"><h1>Request [test]:</h1>%s</div>') % dump_request()
    return (
        htmltext(
            ...
            '</style>'
            '</head><body>%(content)s%(request_html)s</body></html>'
        ) % locals()
    )


def format_request():
    return format_page('Request (session3 tests)', dump_request())


def format_link_list(targets):
    return htmltext('<ul>%s</ul>') % htmltext('').join([htmltext('<li>%s</li>') % href(target, target) for target in targets])
...
class RootDirectory(Directory):
    ...
    def _q_index(self):
        ...
        if sessions:
            content += htmltext('<table><tr><th></th><th>Session</th><th>User</th><th>Number of Requests</th></tr>')
            this_session = get_session()
            for index, (row, session) in enumerate(sessions):
                if session is this_session:
                    formatted_id = htmltext('<span style="font-weight:bold">%s</span>' % row)
                else:
                    formatted_id = row
                content += htmltext('<tr><td>%s</td><td>%s</td><td>%s</td><td>%d</td>'
                    % (index, formatted_id, session.user or htmltext("<em>None</em>"), session.num_requests, )
                )
            content += htmltext('</table>')
        return format_page("Quixote Session-Management Demo/Test", content)
    ...
    def login(self):
        ...
        return format_page("Quixote Session3 Test Demo: Login", content)
    ...
    def logout(self):
        ...
        return format_page("Quixote Session3 Demo: Logout", content)
